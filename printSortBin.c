...
void writeArrayToFile(int arr[], int n, const char *algorithm, int size);  // Modified function
...
int main() {
    ...
    for (i = 0; i < 3; i++) {
        ...
        writeArrayToFile(tempArray, size, "quick_sort", size);  // Save the sorted array
        free(tempArray);
        ...
        writeArrayToFile(tempArray, size, "bubble_sort", size);  // Save the sorted array
        free(tempArray);
        ...
        writeArrayToFile(tempArray, size, "shell_sort", size);  // Save the sorted array
        free(tempArray);
        ...
        writeArrayToFile(tempArray, size, "heap_sort", size);  // Save the sorted array
        free(tempArray);
...
void readArrayFromFile(int arr[], int n, const char *filename) {
    ...
    if (readSize != (size_t)n) {
        printf("Error reading file %s. Expected %d elements.\n", filename, n);
        exit(1);
    }
    fclose(file);
}

// Write data to a text file with a unique name based on algorithm and size
void writeArrayToFile(int arr[], int n, const char *algorithm, int size) {
    char filename[100];
    sprintf(filename, "%s_%d.txt", algorithm, size);  // Create a unique filename
